...
var Document = new Native({
	...
	initialize: function(doc){
		$uid(doc);
		// krs1 begin head = defs and html = svg
		doc.head = doc.getElementsByTagName('defs')[0];
		doc.html = doc.getElementsByTagName('svg')[0];
		// krs1 end
...
Document.implement({
	...
	newElement: function(tag, props){
		...
		return $.element(this.createElementNS("http://www.w3.org/2000/svg",tag)).set(props);
	},
...
Element.implement({
	...
	setProperty: function(attribute, value){
		...
		// krs1 begin
		// baseVal added
		(key) ? this[key].baseVal = value : this.setAttribute(attribute, value);
		// krs1 end
		return this;
	},
...
Native.implement([Document, Element], {
	...
	getElements: function(expression, nocash){
		...
		//if(expression=='circle.Token') alert(items);
...
Selectors.Filters = {
	...
	byClass: function(self, klass){
		...
		for(n in self) {
			if(n=='className') {
				//alert(self.getAttribute('class'));
				//alert('type = '+$type(self[n])+'\nvalue = '+self[n]);
				//var className = self[n];
				//alert(className);
				/*
				for(m in className) {
					alert(m+' : '+className[m]);
		...
		//alert(JSON.encode(self.className.baseVal));
		
		var className = self.getAttribute('class');
		if(className==null) className = false;
		
		return (className && className.contains(klass, ' '));
		
		//return (self.className && (self.className.baseVal)?self.className.baseVal.contains(klass, ' '):false);
	},
