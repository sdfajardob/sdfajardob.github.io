			...
			else 
			{
				...
				if(valid)
				{
					...
					rects.each(function(rect)
					{
						...
						if(inputs.length>0) 
						{
							var fehler = new Array(false,false,false,false);
							inputs.each(function(input)
							{
								...
								if(place) 
								{
									...
									{
										canFire = false;
										fehler[0]="normal arc - firing not possible,\r\nrequired: m(p)>=w.";
									}
									// read arc
									if(arctype=='Read Edge' && marking<multiplicity)
									{
										fehler[1]="read arc - firing not possible,\r\nrequired: m(p)>=w.";
										canFire = false;
									}
									// inhibitor arc
									if(arctype=='Inhibitor Edge' && (marking>=multiplicity))
									{
										fehler[2]="inhibitor arc - firing not possible,\r\nrequired: m(p)<w.";
										canFire = false;
									}
									// equal arc
									if(arctype=='Equal Edge' && (marking!=multiplicity))
									{
										fehler[3]="equal arc - firing not possible,\r\nrequired: m(p)=w.";
										canFire = false;
									}
									// reset arc - no rules needed
							...
							else
							{
								...
								{
									// only on error message pro arctype
		...
		polylines.each(function(polyline)
		{
			...
			switch(arctype)
			{
				case "Edge": 
					...
					break;
				case "Reset Edge": 
					// decrease all token of the place
					decreaseMarking(source, mp);
					break;
				default: // no decrease for other arctypes
			}
		});
	});
  }
